...
int	main(void)
{
	...
	print_sep(S1);
	// =[  ]====================================================================
	print_title("A| NO SPACES OUTSIDE QUOTES");
	t_tok a0[]={{-1,"there_is_no_spaces",0},{0,0,0}};
	nb_err += test("there_is_no_spaces", a0);
	t_tok a1[]={{-1,"there",0},{-1,"'is no'",0},{-1,"spaces",0},{0,0,0}};
	nb_err += test("there'is no'spaces", a1);
	t_tok a2[]={{-1,"there",0},{-1,"\"is no\"",0},{-1,"spaces",0},{0,0,0}};
	nb_err += test("there\"is no\"spaces", a2);
	print_sep(S1);
	// =[  ]====================================================================
	print_title("B| ONLY SPACES");
	t_tok b0[]={{-1," ",0},{0,0,0}};
	print_subtitle("Only spaces");
	nb_err += test(" ", b0);
	nb_err += test("  ", b0);
	nb_err += test("   ", b0);
	print_sep(S2);
	print_subtitle("Only tabulation");
	nb_err += test("\t", b0);
	nb_err += test("\t\t", b0);
	nb_err += test("\t\t\t", b0);
	print_sep(S2);
	print_subtitle("Mix tabulation spaces");
	nb_err += test("\t ", b0);
	nb_err += test(" \t ", b0);
	nb_err += test("\t \t", b0);
	print_sep(S2);
	print_sep(S1);
	// =[  ]====================================================================
	print_title("C| SPACES");
	t_tok c0[]={{-1," ",0},{-1,"start",0},{-1," ",0},{-1,"end",0},{-1," ",0},{0,0,0}};
	nb_err += test(" start end ", c0);
	nb_err += test("    start      end       ", c0);
	t_tok c1[]={{-1," ",0},{-1,"' inside single '",0},{-1," ",0},{-1,"\" inside double \"",0},{-1," ",0},{0,0,0}};
