...
# -[ COMMANDS ]-----------------------------------------------------------------------------------------------
VAL="valgrind --leak-check=full --track-fds=yes --show-leak-kinds=all --error-exitcode=42 --trace-children=yes"
# -[ LISTS ]--------------------------------------------------------------------------------------------------
...
for tst_file in ${TEST_FILE[@]};do
    ...
    $VAL <$tst_file bash --posix >$bash_res_name 2>&1
    $VAL <$tst_file $PROGRAMM >$msll_res_name 2>&1
